...
class Hyperparameters:
    ...
    # Meta-Warmdown (Reptile meta-learning during warmdown phase)
    ...
    meta_inner_steps = int(os.environ.get("META_INNER_STEPS", 1))
    # Legal TTT eval (score-first protocol)
...
class EphemeralLoRA:
    """Lightweight LoRA adapters created/destroyed per meta-step.

    Works with the Parameter Banking architecture: instead of hooks on nn.Linear
    modules, this creates separate A/B tensors per layer for Q, V, and Out
    projections. The GPT.forward_with_lora() method uses these tensors to add
    LoRA perturbations: output += F.linear(F.linear(x, A), B).

    LoRA init: A ~ N(0, 0.01^2), B = 0 (so initial perturbation is zero).
    """
    def __init__(self, num_layers: int, model_dim: int, kv_dim: int, rank: int,
                 device: torch.device, dtype: torch.dtype = torch.bfloat16):
        self.num_layers = num_layers
        self.rank = rank
        self.params: list[Tensor] = []
        # Per-layer LoRA for Q (model_dim -> model_dim), V (model_dim -> kv_dim),
        # Out (model_dim -> model_dim) projections
        # A: (rank, in_dim), B: (out_dim, rank)
        self.lora_q_A: list[Tensor] = []  # (rank, model_dim)
        self.lora_q_B: list[Tensor] = []  # (model_dim, rank)
        self.lora_v_A: list[Tensor] = []  # (rank, model_dim)
        self.lora_v_B: list[Tensor] = []  # (kv_dim, rank)
        self.lora_o_A: list[Tensor] = []  # (rank, model_dim)
        self.lora_o_B: list[Tensor] = []  # (model_dim, rank)
        for _ in range(num_layers):
            # Q projection LoRA
            qA = torch.randn(rank, model_dim, device=device, dtype=dtype) * 0.01
            qB = torch.zeros(model_dim, rank, device=device, dtype=dtype)
            qA.requires_grad_(True); qB.requires_grad_(True)
            self.lora_q_A.append(qA); self.lora_q_B.append(qB)
            self.params.extend([qA, qB])
            # V projection LoRA
            vA = torch.randn(rank, model_dim, device=device, dtype=dtype) * 0.01
            vB = torch.zeros(kv_dim, rank, device=device, dtype=dtype)
            vA.requires_grad_(True); vB.requires_grad_(True)
            self.lora_v_A.append(vA); self.lora_v_B.append(vB)
            self.params.extend([vA, vB])
            # Out projection LoRA
            oA = torch.randn(rank, model_dim, device=device, dtype=dtype) * 0.01
            oB = torch.zeros(model_dim, rank, device=device, dtype=dtype)
            oA.requires_grad_(True); oB.requires_grad_(True)
            self.lora_o_A.append(oA); self.lora_o_B.append(oB)
            self.params.extend([oA, oB])

    def get_layer_lora(self, layer_idx: int) -> dict[str, Tensor]:
        """Return dict of LoRA tensors for a given layer."""
        return {
            'q_A': self.lora_q_A[layer_idx], 'q_B': self.lora_q_B[layer_idx],
            'v_A': self.lora_v_A[layer_idx], 'v_B': self.lora_v_B[layer_idx],
            'o_A': self.lora_o_A[layer_idx], 'o_B': self.lora_o_B[layer_idx],
        }
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor,
                ...
                lora: dict[str, Tensor] | None = None) -> tuple[Tensor, Tensor | None]:
        bsz, seqlen, dim = x.shape
        q = F.linear(x, q_w.to(x.dtype))
        if lora is not None:
            q = q + F.linear(F.linear(x, lora['q_A']), lora['q_B'])
        q = q.reshape(bsz, seqlen, self.num_heads, self.head_dim)
        k = F.linear(x, k_w.to(x.dtype)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        v = F.linear(x, v_w.to(x.dtype))
        if lora is not None:
            v = v + F.linear(F.linear(x, lora['v_A']), lora['v_B'])
        ...
        if lora is not None:
            out = out + F.linear(F.linear(y, lora['o_A']), lora['o_B'])
        return out, raw_v
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor,
        ...
        attn_out, raw_v = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0, lora=lora)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        return main_loss
    def forward_with_lora(self, input_ids: Tensor, target_ids: Tensor,
                          ephemeral_lora: 'EphemeralLoRA') -> Tensor:
        """Forward pass with ephemeral LoRA adapters injected into Q, V, Out projections.
        Used during meta-warmdown inner/outer loops and TTT eval.
        Does NOT include MTP heads (not needed for meta-learning or TTT)."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        ...
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            layer_lora = ephemeral_lora.get_layer_lora(i)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0, lora=layer_lora)
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            ve = self._get_ve(bi, input_ids, ve_cache)
            layer_lora = ephemeral_lora.get_layer_lora(bi)
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                v_embed=ve, v0=v0, lora=layer_lora)
    ...
    def forward_logits_with_lora(self, input_ids: Tensor,
                                  ephemeral_lora: 'EphemeralLoRA') -> Tensor:
        """Return logits with LoRA applied (used by TTT eval scoring)."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
...
def eval_val_sliding_ttt(
    ...
    """Sliding window evaluation with legal score-first Test-Time Training (TTT).

    Protocol (legal, score-first):
    1. Score each chunk under torch.inference_mode() (no LoRA)
    2. Train LoRA adapters on the scored tokens (SGD, multiple epochs)
    3. Use adapted model for the next chunk's scoring

    The LoRA adapters are ephemeral and discarded after each chunk boundary.
    This is legal because scoring always happens BEFORE training on the scored data.
    """
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    ...
    # Partition tokens into chunks for TTT
    ...
    # Create initial LoRA (starts zeroed out, so first chunk is scored without adaptation)
    lora = EphemeralLoRA(args.num_layers, args.model_dim, kv_dim, ttt_rank, device)
    ...
    for chunk_idx in range(my_chunk_start, my_chunk_end):
        ...
        # === STEP 1: Score chunk under inference mode (legal: score BEFORE train) ===
        # Use sliding windows within the chunk
        chunk_window_starts = []
        for ws in range(chunk_start, chunk_end, stride):
            wend = min(ws + seq_len, chunk_end)
            if wend - ws >= 1:
                chunk_window_starts.append(ws)

        with torch.inference_mode():
            for bi in range(0, len(chunk_window_starts), batch_seqs):
                batch_ws = chunk_window_starts[bi:bi + batch_seqs]
                bsz = len(batch_ws)
                x_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
                y_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
                wlens: list[int] = []
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    if chunk_idx > my_chunk_start:
                        # Use LoRA from previous chunk's training
                        logits = base_model.forward_logits_with_lora(x_batch, lora)
                    else:
                ...
                for i, ws in enumerate(batch_ws):
                    wlen = wlens[i]
                    s = 0 if ws == chunk_start and chunk_idx == 0 else max(wlen - stride, 0)
                    scored_nll = nll[i, s:wlen].to(torch.float64)
                    loss_sum += scored_nll.sum()
                    token_count += float(wlen - s)
                    tgt = y_batch[i, s:wlen]
        ...
        # === STEP 2: Train LoRA on scored chunk (legal: training on already-scored data) ===
        if chunk_end < total_tokens:  # Don't train on last chunk (nothing to score after)
            # Re-init LoRA for this chunk's training
            lora = EphemeralLoRA(args.num_layers, args.model_dim, kv_dim, ttt_rank, device)

            # Prepare training data from the chunk
            chunk_data = val_tokens[chunk_start:chunk_end + 1].to(dtype=torch.int64, device=device)
            num_seqs_chunk = max(1, (chunk_end - chunk_start) // seq_len)
            usable = num_seqs_chunk * seq_len
            if usable > 0 and usable < chunk_data.numel():
                train_x = chunk_data[:usable].reshape(num_seqs_chunk, seq_len)
                train_y = chunk_data[1:usable + 1].reshape(num_seqs_chunk, seq_len)

                for epoch in range(ttt_epochs):
                    for seq_idx in range(num_seqs_chunk):
                        x_ttt = train_x[seq_idx:seq_idx + 1]
                        y_ttt = train_y[seq_idx:seq_idx + 1]

                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            ttt_loss = base_model.forward_with_lora(x_ttt, y_ttt, lora)

                        lora_grads = torch.autograd.grad(ttt_loss, lora.params, retain_graph=False)
                        with torch.no_grad():
...
def main() -> None:
    ...
    log0(
        f"meta_warmdown:{args.meta_warmdown} meta_threshold:{args.meta_threshold} "
        f"meta_lora_rank:{args.meta_lora_rank} meta_alpha_inner:{args.meta_alpha_inner} "
        f"meta_inner_steps:{args.meta_inner_steps}"
    )
    log0(
        f"ttt_enabled:{args.ttt_enabled} ttt_lr:{args.ttt_lr} ttt_epochs:{args.ttt_epochs} "
        f"ttt_chunk_tokens:{args.ttt_chunk_tokens} ttt_lora_rank:{args.ttt_lora_rank}"
    )
    ...
    while True:
        ...
        zero_grad_all()
        # === Meta-warmdown decision ===
        use_meta = args.meta_warmdown and scale < args.meta_threshold
        if use_meta and not meta_warmdown_active:
            meta_warmdown_active = True
            log0(f"meta_warmdown:enabled step:{step} scale:{scale:.4f}")
        train_loss = torch.zeros((), device=device)
        if use_meta:
            # === Reptile meta-warmdown training step ===
            # Uses base_model (uncompiled) because LoRA injection is dynamic.
            # torch.compile with dynamic hooks/LoRA injection is not supported.
            head_dim = args.model_dim // args.num_heads
            kv_dim = args.num_kv_heads * head_dim
            for micro_step in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                ...
                B = x.shape[0]
                half = B // 2
                x_sup, y_sup = x[:half], y[:half]
                x_qry, y_qry = x[half:], y[half:]
                # Step 1: Create ephemeral LoRA
                lora = EphemeralLoRA(args.num_layers, args.model_dim, kv_dim,
                                     args.meta_lora_rank, device)
                # Step 2: Inner loop -- adapt LoRA on support set
                for _inner in range(args.meta_inner_steps):
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                        loss_sup = base_model.forward_with_lora(x_sup, y_sup, lora)
                    lora_grads = torch.autograd.grad(loss_sup, lora.params, retain_graph=False)
                    with torch.no_grad():
                        for p, g in zip(lora.params, lora_grads):
                            p.sub_(args.meta_alpha_inner * g)
                # Step 3: Outer loop -- forward with adapted LoRA on query,
                # backward to base model params. LoRA params are NOT in the
                # optimizer param groups so they get no update. The backward
                # computes gradients for base model weights (banks, embeddings,
                # scalars, etc.) which then get processed by Muon/Adam normally.
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    loss_qry = base_model.forward_with_lora(x_qry, y_qry, lora)
                train_loss += loss_qry.detach()
                (loss_qry * grad_scale).backward()
                # Discard LoRA (ephemeral)
                del lora
        else:
    ...
    # === Legal TTT eval (score-first protocol with LoRA adaptation) ===
    if args.ttt_enabled and args.eval_stride > 0:
        torch.cuda.synchronize()
        t_ttt = time.perf_counter()
        log0(f"ttt:starting legal score-first TTT eval (lr={args.ttt_lr} epochs={args.ttt_epochs} "
             f"chunk={args.ttt_chunk_tokens} rank={args.ttt_lora_rank})")
