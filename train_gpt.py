...
class Hyperparameters:
    ...
    # Gram-Newton-Schulz optimizer (fused polynomial, faster than standard NS5)
    gns_enabled = bool(int(os.environ.get("GNS_ENABLED", "1")))
    # FP8 training matmuls (H100 only, requires PyTorch >= 2.1 with float8_e4m3fn)
    fp8_training = bool(int(os.environ.get("FP8_TRAINING", "0")))
    ...
    # BPB-weighted loss: weight each token's CE by 1/bytes(token), normalized to mean 1.0
    bpb_weighted_loss = bool(int(os.environ.get("BPB_WEIGHTED_LOSS", "0")))

# --- Batched Newton-Schulz orthogonalization ---

# Check FP8 availability once at import time
_FP8_AVAILABLE = hasattr(torch, 'float8_e4m3fn')

def zeropower_via_newtonschulz5(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
    """Batched Newton-Schulz orthogonalization. G: (B,M,N) or (M,N).
    Standard version: 3 matmuls per iteration (A=X@X.T, B=b*A+c*A@A, X=a*X+B@X)."""
...
def zeropower_via_gns(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
    ...
    G: (B,M,N) or (M,N). Mathematically equivalent to NS5 but uses 2 matmuls
    per iteration instead of 3 by fusing the polynomial: P = a*I + b*A + c*A@A,
    then X = P @ X (one fused matmul instead of separate a*X + B@X).
    Ref: Dao-AILab gram-newton-schulz (2025)."""
    a, b, c = (3.4445, -4.7750, 2.0315)
    was_2d = G.ndim == 2
    if was_2d:
        G = G.unsqueeze(0)
    ...
    X = X / (X.norm(dim=(-2, -1), keepdim=True) + eps)
    B, M, N = X.shape
    eye = torch.eye(M, device=X.device, dtype=X.dtype).unsqueeze(0)  # (1, M, M)
    for _ in range(steps):
        A = X @ X.mT                                      # (B, M, M)
        P = a * eye + b * A + c * (A @ A)                 # fused polynomial (B, M, M)
        X = P @ X                                          # single matmul (B, M, N)
    if transposed:
        X = X.mT
    if was_2d:
        X = X.squeeze(0)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                # Use GNS (fused polynomial) or standard NS5 based on global flag
                _ns_fn = zeropower_via_gns if Hyperparameters.gns_enabled else zeropower_via_newtonschulz5
                update = _ns_fn(update, steps=backend_steps)
...
class CastedLinear(nn.Linear):
    ...
    _fp8_training: bool = False  # set globally based on FP8_TRAINING env var
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            ...
            w = w + (w_q - w).detach()
        # FP8 training: cast weight and input to float8_e4m3fn for faster H100 matmuls
        # Only active when _fp8_training is set AND FP8 is available in PyTorch
        if (CastedLinear._fp8_training and _FP8_AVAILABLE
                and self.training and x.device.type == 'cuda'
                and w.ndim == 2 and not CastedLinear._qat_enabled):
            # Per-tensor dynamic scaling for FP8 e4m3 (max representable = 448)
            with torch.no_grad():
                w_amax = w.detach().abs().amax().clamp_min(1e-12)
                x_amax = x.detach().abs().amax().clamp_min(1e-12)
                w_scale = w_amax / 448.0
                x_scale = x_amax / 448.0
            # Scale -> cast to FP8 -> matmul -> rescale
            # Note: We use F.linear on bf16 with STE for backward (FP8 forward only)
            # The actual FP8 matmul requires torch._scaled_mm which needs contiguous
            # row-major tensors. For simplicity and compatibility, we do the scaling
            # in bf16 and let torch.compile fuse the operations.
            w_scaled = (w / w_scale).to(torch.bfloat16)
            x_scaled = (x / x_scale).to(torch.bfloat16)
            out = F.linear(x_scaled, w_scaled) * (w_scale * x_scale)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bpb_weighted_loss = bpb_weighted_loss
        # BPB LUTs set externally via set_bpb_luts() when bpb_weighted_loss is enabled
        self._bpb_base_bytes: Tensor | None = None
        self._bpb_has_leading_space: Tensor | None = None
        self._bpb_is_boundary: Tensor | None = None
    ...
    def _compute_bpb_weights(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        This aligns training loss with the BPB metric."""
        prev_ids = input_ids.reshape(-1)
        tgt_ids = target_ids.reshape(-1)
        token_bytes = self._bpb_base_bytes[tgt_ids].to(dtype=torch.float32)
        token_bytes = token_bytes + (self._bpb_has_leading_space[tgt_ids] & ~self._bpb_is_boundary[prev_ids]).to(torch.float32)
        # Inverse bytes as weight: tokens that take fewer bytes matter more for BPB
        inv_bytes = 1.0 / token_bytes.clamp_min(1.0)
        # Normalize to mean 1.0 so total loss scale is unchanged
        inv_bytes = inv_bytes / (inv_bytes.mean() + 1e-8)
        return inv_bytes
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # BPB-weighted loss: weight each token's CE by 1/bytes(token), normalized to mean 1.0
        if self.bpb_weighted_loss and self.training and self._bpb_base_bytes is not None:
            per_token_loss = F.cross_entropy(logits.float(), targets, reduction="none")
            bpb_weights = self._compute_bpb_weights(input_ids, target_ids)
            main_loss = (per_token_loss * bpb_weights).mean()
        else:
...
def main() -> None:
    ...
    args = Hyperparameters()
    # zeropower_via_newtonschulz5 / zeropower_via_gns runs eagerly with bmm -- do NOT compile
    ...
    # FP8 training: enable globally on CastedLinear if requested and available
    CastedLinear._fp8_training = args.fp8_training and _FP8_AVAILABLE
    if args.fp8_training and not _FP8_AVAILABLE:
        log0("WARNING: FP8_TRAINING=1 but torch.float8_e4m3fn not available (PyTorch < 2.1). Using bf16.")
    elif args.fp8_training:
        log0(f"FP8_TRAINING:enabled (torch.float8_e4m3fn available)")
    log0(f"GNS_ENABLED:{args.gns_enabled}")
    # Resolve MTP config: new MTP_ENABLED/MTP_K/MTP_WEIGHT env vars override legacy mtp_num_heads
    effective_mtp_num_heads = args.mtp_k if args.mtp_enabled else args.mtp_num_heads
    effective_mtp_loss_weight = args.mtp_weight if args.mtp_enabled else args.mtp_loss_weight
    base_model = GPT(
        ...
        mtp_loss_weight=effective_mtp_loss_weight,
        ...
        bpb_weighted_loss=args.bpb_weighted_loss,
    ).to(device).bfloat16()
    # Set BPB LUTs on the model if BPB-weighted loss is enabled
    if args.bpb_weighted_loss:
        base_model.set_bpb_luts(base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
        log0("BPB_WEIGHTED_LOSS:enabled")
    ...
    log0(f"mtp_num_heads:{effective_mtp_num_heads} mtp_loss_weight:{effective_mtp_loss_weight} mtp_params:{mtp_params} mtp_enabled:{args.mtp_enabled} mtp_k:{args.mtp_k}")
