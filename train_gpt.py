...
class Hyperparameters:
    ...
    mlp_rank = int(os.environ.get("MLP_RANK", 0))  # 0 = full rank (no change); >0 = low-rank factored MLP
...
class MLP(nn.Module):
    ...
    def forward_lowrank(self, x: Tensor, up_A: Tensor, up_B: Tensor, down_A: Tensor, down_B: Tensor) -> Tensor:
        """Low-rank factored MLP: W_up = up_B @ up_A^T (hidden,dim), W_down = down_B @ down_A^T (dim,hidden).
        up_A: (dim, rank), up_B: (hidden, rank), down_A: (hidden, rank), down_B: (dim, rank)."""
        h = x @ up_A.to(x.dtype)              # (*, dim) @ (dim, rank) -> (*, rank)
        h = F.linear(h, up_B.to(x.dtype))     # (*, rank) via F.linear(w=(hidden, rank)) -> (*, hidden)
        h = F.leaky_relu(h, negative_slope=0.5)
        h = h.square()
        h = h @ down_A.to(x.dtype)            # (*, hidden) @ (hidden, rank) -> (*, rank)
        return F.linear(h, down_B.to(x.dtype)) # (*, rank) via F.linear(w=(dim, rank)) -> (*, dim)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        return x_out, raw_v
    def forward_lowrank(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor,
                        up_A: Tensor, up_B: Tensor, down_A: Tensor, down_B: Tensor,
                        v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, raw_v = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp.forward_lowrank(self.mlp_norm(x_out) * self.ln_scale_factor, up_A, up_B, down_A, down_B)
        if self.dtg_gate is not None:
            gate = torch.sigmoid(self.dtg_gate(x_in.detach()))
            x_out = x_in + gate * (x_out - x_in)
        return x_out, raw_v
...
class GPT(nn.Module):
    def __init__(
        ...
        if mlp_rank > 0:
            # Low-rank factored MLP banks: W_up = B_up @ A_up^T, W_down = B_down @ A_down^T
            # up_A: (layers, dim, rank), up_B: (layers, hidden, rank) -> W_up = up_B @ up_A^T is (hidden, dim)
            # down_A: (layers, hidden, rank), down_B: (layers, dim, rank) -> W_down = down_B @ down_A^T is (dim, hidden)
            self.mlp_up_A_bank = nn.Parameter(torch.empty(num_layers, model_dim, mlp_rank))
            self.mlp_up_B_bank = nn.Parameter(torch.empty(num_layers, mlp_dim, mlp_rank))
            self.mlp_down_A_bank = nn.Parameter(torch.empty(num_layers, mlp_dim, mlp_rank))
            self.mlp_down_B_bank = nn.Parameter(torch.empty(num_layers, model_dim, mlp_rank))
        else:
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(n):
            ...
            if self.mlp_rank > 0:
                # Low-rank factors: orthogonal init
                nn.init.orthogonal_(self.mlp_up_A_bank.data[i])    # (dim, rank)
                nn.init.orthogonal_(self.mlp_up_B_bank.data[i])    # (hidden, rank)
                # Down factors: zero-init the output factor, orthogonal the input factor
                nn.init.orthogonal_(self.mlp_down_A_bank.data[i])  # (hidden, rank)
                nn.init.zeros_(self.mlp_down_B_bank.data[i])        # (dim, rank) -- zero init output
                self.mlp_down_B_bank.data[i].mul_(proj_scale)
            else:
    ...
    def _block_forward(self, i: int, x: Tensor, x0: Tensor, v_embed: Tensor | None, v0: Tensor | None) -> tuple[Tensor, Tensor | None]:
        """Dispatch to full-rank or low-rank block forward for layer i."""
        n = self.num_layers
        if self.mlp_rank > 0:
            return self.blocks[i].forward_lowrank(x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i],
                self.mlp_up_A_bank[i], self.mlp_up_B_bank[i],
                self.mlp_down_A_bank[i], self.mlp_down_B_bank[i],
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self._block_forward(i, x, x0, v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self._block_forward(bi, x, x0, v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int) -> dict[str, Tensor]:
    ...
    For low-rank models, materialize W = B @ A^T for GPTQ quantization."""
    out: dict[str, Tensor] = {}
    n = num_layers
    has_lowrank = "mlp_up_A_bank" in sd
    for name, tensor in sd.items():
        ...
        elif name == "mlp_up_A_bank":
            # Materialize W_up = B_up @ A_up^T for each layer
            up_B = sd["mlp_up_B_bank"]  # (layers, hidden, rank)
            for i in range(n):
                # W_up = up_B[i] @ up_A[i]^T -> (hidden, dim)
                out[f"blocks.{i}.mlp.fc.weight"] = up_B[i].float() @ tensor[i].float().T
        elif name == "mlp_down_A_bank":
            # Materialize W_down = B_down @ A_down^T for each layer
            down_B = sd["mlp_down_B_bank"]  # (layers, dim, rank)
            for i in range(n):
                # W_down = down_B[i] @ down_A[i]^T -> (dim, hidden)
                out[f"blocks.{i}.mlp.proj.weight"] = down_B[i].float() @ tensor[i].float().T
        elif name in ("mlp_up_B_bank", "mlp_down_B_bank"):
            pass  # handled above together with A banks
        else:
            out[name] = tensor
    return out
...
def main() -> None:
    ...
    if args.mlp_rank > 0:
        base_model.mlp_up_A_bank.data = base_model.mlp_up_A_bank.data.float()
        base_model.mlp_up_B_bank.data = base_model.mlp_up_B_bank.data.float()
        base_model.mlp_down_A_bank.data = base_model.mlp_down_A_bank.data.float()
        base_model.mlp_down_B_bank.data = base_model.mlp_down_B_bank.data.float()
    else:
    ...
    if args.mlp_rank > 0:
        matrix_params = [
            ...
            base_model.mlp_up_A_bank, base_model.mlp_up_B_bank,
            base_model.mlp_down_A_bank, base_model.mlp_down_B_bank,
        ]
    else:
        matrix_params = [
            base_model.qo_bank, base_model.kv_bank,
    ...
    if args.mlp_rank > 0:
        ...
        lowrank_mlp_params = args.num_layers * 2 * (args.model_dim * args.mlp_rank + mlp_dim * args.mlp_rank)
        fullrank_mlp_params = args.num_layers * 2 * args.model_dim * mlp_dim
        log0(f"mlp_rank:{args.mlp_rank} lowrank_mlp_params:{lowrank_mlp_params} fullrank_equiv:{fullrank_mlp_params} savings:{fullrank_mlp_params - lowrank_mlp_params}")
    ...
    # Eval model always uses full-rank banks (materialized from factors if low-rank training)
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, model_dim=args.model_dim,
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult,
        ...
        mlp_rank=0,  # eval model always full-rank (materialized products)
    ).to(device).bfloat16()
    eval_template_sd = {k: v.detach().cpu() for k, v in eval_model.state_dict().items()}
    # Re-bank the dequantized tensors using eval model's template
