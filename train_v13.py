...
# --- Muon optimizer (PR #1394 style: per-layer NS5, round-robin distributed) ---
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.partial_key_offset = False
        self.sparse_attn_gate = None
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None, attn_input_override: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_src = attn_input_override if attn_input_override is not None else x_in
        attn_out, raw_v = self.attn(self.attn_norm(attn_src) * self.ln_scale_factor, v_embed=v_embed, v0=v0)
        if self._parallel:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                train_seq_len=train_seq_len, layer_idx=i, ln_scale=ln_scale, dtg=dtg,
                gated_attention=gated_attention, value_residual=value_residual,
            )
        ...
        if backout_layers > 0:
            ...
            self._backout_save_at = num_layers - backout_layers
        else:
    ...
    def _run_blocks(self, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict) -> Tensor:
        ...
        for bi in enc_iter:
            ...
            x, raw_v = self.blocks[bi](x, x0, v_embed=ve, v0=v0, attn_input_override=attn_override)
        ...
        for skip_idx, bi in enumerate(dec_iter):
            ...
            ve = self._get_ve(bi, input_ids, ve_cache)
            attn_override = x_backout if (x_backout is not None and bi >= self._backout_save_at) else None
            x, _ = self.blocks[bi](x, x0, v_embed=ve, v0=v0, attn_input_override=attn_override)
...
def eval_val_sliding_ttt(
    ...
    ttt_params = [
        ...
        and "tok_emb" not in n and "bigram" not in n and "ve_shared.embed" not in n
    ]
    ...
    for ci in range(num_chunks):
        ...
        # Clear RoPE caches so they regenerate outside inference_mode
...
# _HessianAttn/MLP/Block/GPT removed — bankless model uses CastedLinear directly
def collect_hessians(hessian_model, train_loader, args, device, grad_accum_steps, num_batches=256):
    """Run calibration batches, collecting H = X^T X for each CastedLinear."""
    hessians = {}
    hooks = []
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessians: dict[str, Tensor] | None = None, int8_layers: set[int] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1 and not force_int8:
            ...
            H = hessians.get(name) if hessians else None
            if cat == "embed" and H is not None:
                q, s = quantize_int6_gptq(t, hessian=H, clip_range=127, clip_sigmas=EMBED_CLIP_SIGMAS)
            else:
                q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def main() -> None:
    ...
    # Optimizer split (PR #1394 style):
    # - block matrix params (CastedLinear weights) -> Muon (per-layer NS5)
    ...
    while True:
        ...
        if not base_model.looping_active and args.recur_layers:
            ...
            if current_progress >= recur_threshold:
                ...
                log0(f"looping:activated step:{step} time:{approx_training_time_ms:.0f}ms encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}")
    ...
    # GPTQ: collect Hessians by hooking CastedLinear on the training model directly (PR #1394 style)
    calib_recurrence = bool(int(os.environ.get("CALIB_RECURRENCE", "1")))
    if args.recur_layers and calib_recurrence:
        base_model.looping_active = True
        log0(f"gptq:looping_active=True for calibration (recur_layers={args.recur_layers})")
    else:
        base_model.looping_active = False
        log0(f"gptq:looping_active=False for calibration (CALIB_RECURRENCE={int(calib_recurrence)})")
    _emb_H_accum = [torch.zeros(args.model_dim, args.model_dim, dtype=torch.float32, device='cpu')]
    _emb_count = [0]
    def _emb_hessian_hook(module, input, output):
        x = output.detach().float()
        if x.ndim == 3:
            x = x.reshape(-1, x.shape[-1])
        _emb_H_accum[0] += (x.T @ x).cpu()
        _emb_count[0] += 1
    if args.tie_embeddings:
        _emb_hook_handle = base_model.final_norm.register_forward_hook(_emb_hessian_hook)
        log0("gptq:registered embedding Hessian hook on final_norm")
    log0(f"gptq:collecting hessians from training data ({args.gptq_calib_batches} batches)...")
    t_gen = time.perf_counter()
    hessians = collect_hessians(base_model, train_loader, args, device, grad_accum_steps, num_batches=args.gptq_calib_batches)
    log0(f"gptq:collected hessians for {len(hessians)} layers in {time.perf_counter()-t_gen:.1f}s")
    if args.tie_embeddings:
        _emb_hook_handle.remove()
        if _emb_count[0] > 0:
            emb_H = _emb_H_accum[0] / _emb_count[0]
            damp = 0.01 * torch.diag(emb_H).mean().clamp_min(1e-6)
            emb_H += damp * torch.eye(emb_H.shape[0])
            hessians["tok_emb.weight"] = emb_H
            log0(f"gptq:collected embedding Hessian from {_emb_count[0]} batches")
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, hessians=hessians, int8_layers=recur_int8_layers)
    ...
    eval_model = GPT(
        ...
        train_seq_len=args.train_seq_len, mtp_num_heads=0, mtp_loss_weight=0.0,
    ...
    if args.recur_layers and not args.recur_skip_eval:
        eval_model.looping_active = True
        log0(f"eval: looping_active=True (recur_layers={args.recur_layers})")
    elif args.recur_skip_eval:
        eval_model.looping_active = False
        log0("eval: looping disabled for evaluation (recur_skip_eval=1)")
