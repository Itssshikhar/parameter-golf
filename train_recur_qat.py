...
class Hyperparameters:
    ...
    qat_with_recurrence = bool(int(os.environ.get("QAT_WITH_RECURRENCE", "0")))
...
def _noisy_quantize_int6(w: Tensor) -> Tensor:
    """Noisy QAT: add uniform noise matching int6 step size (PR #363 approach).
    Forward sees w + noise (differentiable), backward gets real gradients.
    Trains weights to be robust to quantization noise through recurrence."""
    w32 = w.float()
    row_max = w32.abs().amax(dim=1, keepdim=True)
    scale = (row_max / 31.0).clamp_min(1.0 / 31.0)
    noise = (torch.rand_like(w32) - 0.5) * scale
    w_noisy = torch.clamp(w32 + noise, -31.0 * scale, 31.0 * scale)
    return w_noisy.to(w.dtype)
...
class _HessianAttn(nn.Module):
    ...
    def __init__(self, dim, num_heads, num_kv_heads, rope_base, qk_gain_init):
        ...
        self.partial_key_offset = False  # set by _HessianGPT for full-attention layers
...
def main() -> None:
    ...
    while True:
        ...
        if not base_model.recur_active and base_model._recur_encoder_indices is not None:
            ...
            if current_progress >= recur_threshold:
                ...
                if args.qat_with_recurrence and not CastedLinear._qat_enabled:
                    CastedLinear._qat_enabled = True
                    if args.noisy_qat:
                        _USE_NOISY_QAT = True
                    log0(f"qat_with_recurrence:enabled step:{step} noisy={args.noisy_qat}")
    ...
    log0(f"gptq:AR generation base_model.recur_active={base_model.recur_active}")
